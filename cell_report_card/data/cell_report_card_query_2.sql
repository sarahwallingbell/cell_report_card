    ...
    left join tissue_processings tp on slice.tissue_processing_id = tp.id
    left join task_templates tpt on tp.tissue_processing_template_id = tpt.id and tpt.type = 'TissueProcessingTemplate'
    left join tcp_sops tsop on tpt.tcp_sop_id = tsop.id
    left join cell_reporters cellr on cell.cell_reporter_id = cellr.id
    left join ephys_roi_results err on err.id = cell.ephys_roi_result_id
    ...
    select max(id) as image_series_id, max(workflow_state) as workflow_state, specimen_id
    from image_series
    where is_stack = true
    ...
    left join ephys_roi_results err on err.id = cell.ephys_roi_result_id
    left join specimen_tags_specimens sptagsp on sptagsp.specimen_id = cell.id
    left join specimen_tags tag on tag.id = sptagsp.specimen_tag_id and tag.id in (602120185,602122082)
    group by cell.id
    order by 1
    ...
    where
    proj.code ilike any (array['hIVSCC-METx', 'hIVSCC-MET', 'hIVSCC-METc', 'hIVSCC-MET-HM', 'hIVSCC-MET-GT', 'hIVSCC-METc-HM', 'hIVSCC-METc-GT', 'MET-NM'])
